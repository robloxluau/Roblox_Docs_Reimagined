...
| `__eq(table1, table2)` | The `==` equality operator, also overloads `~=` and inverts the return of the function. |
...
`__eq` overloads `==` and `~=`, when `~=` is used, the return of the function is inverted

```lua filename="ServerScriptService__eqAnd__neq.lua
...
print(Vector(5, 9) == Vector(5, 9)) --> true
print(Vector(5, 9) == Vector(0, 0)) --> false
print(Vector(5, 9) ~= Vector(0, 0)) --> true
```

`__lt` and `__le` are used to overload `<` and `<=` respectively.

```lua filename="ServerScriptService__ltAnd__le.lua
local function Vector(x, y)
  local vec = {}
  vec.X = x
  vec.Y = y
...
print(Vector(5, 9) < Vector(5, 9)) --> false
print(Vector(5, 9) <= Vector(5, 9)) --> true
print(Vector(0, 0) < Vector(5, 9)) --> true
```

<Callout type='info'>
  The `__lt` and `__le` metamethods will always invoke on the left-hand side's object. Meaning, for example, `a > b` will fire the `b`'s `__lt` metamethod. The function between both metatables must match.
</Callout>
